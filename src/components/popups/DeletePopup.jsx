...
import { blockUser } from '../../services/apis';

function DeletePopup({ showDelete, setShowDelete, rows, selectedRowIndex, isBlock, setBlockedDone }) {
    ...
    const handleBlock = async () => {
        ...
        try {
            ...
            const respose = await blockUser(data, headers);
            setBlockedDone(!isBlock);
            ...
            {showDelete &&
                <div className="fixed inset-0 h-screen w-screen backdrop-blur-md z-100 flex items-center justify-center">
                    <div className="relative w-[700px] h-[450px] border border-white shadow-[0px_0px_20px_20px_rgba(255,255,255,0.37)] rounded-[40px] bg-[#1F2D68] flex flex-col justify-around items-center p-12">
                        ...
                        <div className="text-[29.23px] text-white">{`SURE YOU WANT TO ${isBlock ? "UNBLOCK" : "BLOCK"} ?`}</div>
                        <div className="flex gap-10">
                            <button
                                onClick={() => { setShowDelete(false); handleBlock() }}
